...
AAuraCharacterBase::AAuraCharacterBase()
{
	...
	HaloOfProtectedNiagaraComponent = CreateDefaultSubobject<UPassiveNiagaraComponent>("HaloOfProtectionComp");
	HaloOfProtectedNiagaraComponent->SetupAttachment(EffectAttachComponent);
...
void AAuraCharacterBase::SetIsBeingShockLoop_Implementation(bool InShockLoop)
{
	bIsBeingShock = InShockLoop;
	GetCharacterMovement()->MaxWalkSpeed = bIsBeingShock ? 0 : BaseWalkSpeed;
}
...
void AAuraCharacterBase::InitializeAttributesFromSaveData(ULoadScreenSaveGame* SaveGame) const
{
	check(IsValid(GetAbilitySystemComponent()));
	const FAuraGameplayTags& GameplayTags = FAuraGameplayTags::Get();
	...
	const FGameplayEffectSpecHandle SpecHandle = GetAbilitySystemComponent()->MakeOutgoingSpec(PrimaryAttributes_SetByCaller, 1.f, EffectContextHandle);
